...
public class Server {
    ...
    private final ServerChat serverChat; // Módulo de chat
    ...
    public void start() {
        ...
        while (running) {
            try {
                // Bloqueia até um cliente se conectar e retorna um socket dedicado àquele cliente:
                Socket clientSocket = serverSocket.accept();

                // Cria um handler dedicado para o cliente e adiciona à lista global:
                ClientHandler handler = new ClientHandler(clientSocket, activeClients, serverChat);
                activeClients.add(handler);

                // Inicia a thread do handler para processar as requisições do cliente de forma independente:
